//
//  FourPlayerView.swift
//  DartCounter
//
//  Created by Jonas Schlauch on 12.11.20.
//

import UIKit

class FourPlayerView: UIView {
    
    @IBOutlet weak var label_player1_name: UILabel!
    @IBOutlet weak var label_player1_sets: UILabel!
    @IBOutlet weak var label_player1_legs: UILabel!
    @IBOutlet weak var label_player1_pointsLeft: UILabel!
    @IBOutlet weak var label_player1_lastThrow: UILabel!
    @IBOutlet weak var label_player1_dartsThrown: UILabel!
    @IBOutlet weak var label_player1_average: UILabel!
    @IBOutlet weak var label_player1_checkoutPercentage: UILabel!
    
    @IBOutlet weak var label_player2_name: UILabel!
    @IBOutlet weak var label_player2_sets: UILabel!
    @IBOutlet weak var label_player2_legs: UILabel!
    @IBOutlet weak var label_player2_pointsLeft: UILabel!
    @IBOutlet weak var label_player2_lastThrow: UILabel!
    @IBOutlet weak var label_player2_dartsThrown: UILabel!
    @IBOutlet weak var label_player2_average: UILabel!
    @IBOutlet weak var label_player2_checkoutPercentage: UILabel!
    
    @IBOutlet weak var label_player3_name: UILabel!
    @IBOutlet weak var label_player3_sets: UILabel!
    @IBOutlet weak var label_player3_legs: UILabel!
    @IBOutlet weak var label_player3_pointsLeft: UILabel!
    @IBOutlet weak var label_player3_lastThrow: UILabel!
    @IBOutlet weak var label_player3_dartsThrown: UILabel!
    @IBOutlet weak var label_player3_average: UILabel!
    @IBOutlet weak var label_player3_checkoutPercentage: UILabel!
    
    @IBOutlet weak var label_player4_name: UILabel!
    @IBOutlet weak var label_player4_sets: UILabel!
    @IBOutlet weak var label_player4_legs: UILabel!
    @IBOutlet weak var label_player4_pointsLeft: UILabel!
    @IBOutlet weak var label_player4_lastThrow: UILabel!
    @IBOutlet weak var label_player4_dartsThrown: UILabel!
    @IBOutlet weak var label_player4_average: UILabel!
    @IBOutlet weak var label_player4_checkoutPercentage: UILabel!


    override init(frame: CGRect) {
        super.init(frame: frame)
        self.initView()
    }
    
    required init?(coder: NSCoder) {
        super.init(coder: coder)
        self.initView()
    }
    
    private func initView() {
        guard let view = self.loadViewFromNib(nibName: "FourPlayerView") else { return }
        self.addSubview(view)
        self.refreshView()
    }
    
    func refreshView() {
        let player1 = App.game!.players[0]
        let player2 = App.game!.players[1]
        let player3 = App.game!.players[2]
        let player4 = App.game!.players[3]
        
        if player1.isNext! {
            // TODO
        } else {
            // TODO
        }
        
        if player2.isNext! {
            // TODO
        } else {
            // TODO
        }
        
        if player3.isNext! {
            // TODO
        } else {
            // TODO
        }
        
        if player4.isNext! {
            // TODO
        } else {
            // TODO
        }
        
        label_player1_name.text = player1.name
        label_player1_sets.text = "S: " + String(player1.sets ?? -1)
        label_player1_legs.text = "L: " + String(player1.legs!)
        label_player1_pointsLeft.text = String(player1.pointsLeft!)
        label_player1_lastThrow.text = String(player1.lastThrow ?? -1)
        label_player1_dartsThrown.text = String(player1.dartsThrown!)
        
        label_player2_name.text = player2.name
        label_player2_sets.text = "S: " + String(player2.sets ?? -1)
        label_player2_legs.text = "L: " + String(player2.legs!)
        label_player2_pointsLeft.text = String(player2.pointsLeft!)
        label_player2_lastThrow.text = String(player2.lastThrow ?? -1)
        label_player2_dartsThrown.text = String(player2.dartsThrown!)
        
        label_player3_name.text = player3.name
        label_player3_sets.text = "S: " + String(player3.sets ?? -1)
        label_player3_legs.text = "L: " + String(player3.legs!)
        label_player3_pointsLeft.text = String(player3.pointsLeft!)
        label_player3_lastThrow.text = String(player3.lastThrow ?? -1)
        label_player3_dartsThrown.text = String(player3.dartsThrown!)
        
        label_player4_name.text = player4.name
        label_player4_sets.text = "S: " + String(player4.sets ?? -1)
        label_player4_legs.text = "L: " + String(player4.legs!)
        label_player4_pointsLeft.text = String(player4.pointsLeft!)
        label_player4_lastThrow.text = String(player4.lastThrow ?? -1)
        label_player4_dartsThrown.text = String(player4.dartsThrown!)
        
        
        if App.settings.showAverage {
            label_player1_average.text = String(player1.average!)
            label_player2_average.text = String(player2.average!)
            label_player3_average.text = String(player3.average!)
            label_player4_average.text = String(player4.average!)
        } else {
            label_player1_average.isHidden = true
            label_player2_average.isHidden = true
            label_player3_average.isHidden = true
            label_player4_average.isHidden = true
        }
        
        if App.settings.showCheckoutPercentage {
            label_player1_checkoutPercentage.text = String(player1.checkoutPercentage!)
            label_player2_checkoutPercentage.text = String(player2.checkoutPercentage!)
            label_player3_checkoutPercentage.text = String(player3.checkoutPercentage!)
            label_player4_checkoutPercentage.text = String(player4.checkoutPercentage!)
        } else {
            label_player1_checkoutPercentage.isHidden = true
            label_player2_checkoutPercentage.isHidden = true
            label_player3_checkoutPercentage.isHidden = true
            label_player4_checkoutPercentage.isHidden = true
        }
    }

}
